...
| Memory Graph v3 | `ai_engineering/graph_contract.py`, `schemas/memory-graph-contract-v1.schema.json`, `tests/ai_engineering/test_graph_contract.py`, `docs/adr/ADR-0081-memory-graph-engineering-v3.md`, `gateway/memory/graph_projection.py`, `tests/gateway/memory/test_graph_projection.py`, `docs/adr/ADR-0082-deterministic-graph-projection-engine.md`, `gateway/memory/graph_query.py`, `tests/gateway/memory/test_graph_query.py`, `docs/adr/ADR-0084-deterministic-graph-read-path.md` | Deterministic graph contracts, strict authoritative (SQLite) to derived-graph boundary, canonical complete `O(n)` source-state freshness, pure structural projection queries, fail-closed graph validation, provenance-bound authoritative hydration, deterministic ordering, and read-only adversarial evidence |
