...
def test_format_nutrition_diary_report_localized_and_html_safe():
    ...
    assert "???? <b>???????? ?????????????? ???? ??????????????:</b>" in report
    assert "???? ??????????????: 520 ????????" in report
    assert "?????????? &amp; ?????????? &lt;??????????&gt;" in report
    assert "???? <b>???????????? ????????:</b>" in report
...
async def test_telegram_stats_7d_command_routes_correctly(monkeypatch):
    ...
    def _fake_summary(*args, **kwargs):
        ...
        return {
            "entries": [{"meal_name": "??????", "calories_kcal": 300, "protein_g": 12, "fat_g": 8, "carbs_g": 40}],
            "entry_count": 1,
            "calories_kcal": 2100,
            "protein_g": 84,
            "fat_g": 56,
    ...
    monkeypatch.setattr(
        "gateway.platforms.telegram.format_nutrition_diary_report",
        lambda summary: f"???? <b>???????? ???????????????????? ???? {summary['days']} ????????:</b>",
    )
