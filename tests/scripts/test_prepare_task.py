...
def test_prepare_task_with_intent(tmp_path: Path) -> None:
    root = _repository(tmp_path)
    ...
    intent_path.write_text(json.dumps(intent_payload))
    context = build_task_context(root, intent_path=intent_path)
    assert "intent" in context
    assert context["intent"]["task_id"] == "TASK-123"
    assert context["intent"]["source_base_sha"] == head_sha


def test_prepare_task_with_intent_sha_mismatch(tmp_path: Path, capsys) -> None:
    root = _repository(tmp_path)
    intent_payload = {
        "schema_version": 1,
        "task_id": "TASK-123",
    ...
    assert result == 2
    captured = capsys.readouterr()
    assert "INTENT_BASE_SHA_MISMATCH" in captured.err
