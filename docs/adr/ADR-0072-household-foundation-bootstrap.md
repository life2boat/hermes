...
Recommended implementation: lowercase canonical UUIDv4 text, generated by
the trusted household store/service boundary. Equivalent opaque IDs are allowed
only if they have comparable entropy, fixed nonsemantic length, and no ordering
or actor-count leakage. Do not derive the ID from Telegram ID, legacy user ID,
sequential counters, household counts, timestamps, or usernames. Idempotency is
provided by unique constraints and transactional get-or-create, not by reversible
IDs. On generated-ID collision, retry inside the transaction budget and then stop
with a safe aggregate error type. Deterministic tests may inject a generator.
...
For every active household there must be exactly one active owner member, and
`households.owner_user_id` must equal that owner member's `linked_user_id`.
Disabled or closed households may preserve historical ownership metadata, but
new household-aware writes must reject ambiguous ownership. Owner transfer is a
single transaction that updates the owner member role/status and household
pointer together. A controlled repair path may fix owner-pointer mismatches only
after a read-only audit reports safe aggregates.
...
household.owner_user_id equals the active owner member linked_user_id
```
...
- nonpositive, bot, system, or test actor where metadata marks the actor unsafe.
