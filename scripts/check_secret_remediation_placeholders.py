...
MUTATION_CRITICAL_MODULES = [
    ...
    "ops/secret_remediation_r1/json_override.py",
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef) and node.name.startswith("test_"):
                    ...
                    if not statements:
                        findings.append(
                            f"{fpath.relative_to(root)}:{node.lineno}: Empty test (only docstring)"
                        )
                        continue
                    if len(statements) == 1:
                        stmt = statements[0]
                        if isinstance(stmt, ast.Pass):
                            findings.append(
                                f"{fpath.relative_to(root)}:{node.lineno}: Empty test (pass)"
                            )
                        elif (
                            ...
                            findings.append(
                                f"{fpath.relative_to(root)}:{node.lineno}: Empty test (ellipsis)"
                            )
                        elif (
                            ...
                            findings.append(
                                f"{fpath.relative_to(root)}:{node.lineno}: Empty test (assert True)"
                            )
                        elif (
                            isinstance(stmt, ast.Expr)
                            ...
                            findings.append(
                                f"{fpath.relative_to(root)}:{node.lineno}: Empty test (only pytest.skip)"
                            )
