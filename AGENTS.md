...
* выполняется внутри разрешённого repository/worktree;

выполняй его сразу без вопросов.

Не останавливайся после каждого шага. Самостоятельно изучай проект, составляй план, вноси изменения, запускай проверки, исправляй найденные ошибки и доводи задачу до указанной точки остановки.

### Не спрашивай подтверждение для

Выполняй без согласования:

* чтение файлов проекта;
* поиск по коду;
* изучение Git history и diff;
* создание отдельного Git worktree;
* создание рабочей ветки;
* редактирование разрешённых файлов;
* создание новых исходных файлов и тестов;
* форматирование кода;
* запуск линтеров;
* запуск type checking;
* запуск unit и integration tests;
* запуск `git diff --check`;
* запуск существующих безопасных project checks;
* исправление ошибок, обнаруженных тестами;
* повторный запуск проверок;
* создание обычного Git commit;
* push рабочей ветки в разрешённый remote;
* создание Draft PR;
* обновление Draft PR;
* чтение состояния CI;
* подготовка отчётов, runbooks и документации;
* read-only проверка Docker, SQLite, filesystem и запущенных сервисов;
* создание SQLite backup через SQLite backup API, когда это прямо требуется задачей;
* безопасные операции внутри временной тестовой БД.

Не спрашивай разрешение на команды только потому, что они используют:
...
### Спрашивай подтверждение только перед опасными действиями

Остановись и запроси одно понятное подтверждение только перед:

* удалением или очисткой пользовательских данных;
* изменением production DB;
* восстановлением production DB из backup;
* destructive SQL;
* `git reset --hard` в существующем checkout;
* `git clean`;
* force push;
* удалением branch или worktree;
* удалением Docker images, containers, volumes или build cache;
* `docker system prune`;
* удалением backups или logs;
* изменением `.env`, secrets, tokens или credentials;
* выводом секретов;
* изменением firewall, SSH, сетевых правил или системных пользователей;
* reboot или shutdown сервера;
* изменением размера диска или разделов;
* `growpart`, `resize2fs`, `parted`, `fdisk` с записью;
* production build, deploy, restart или container recreate, если они прямо не разрешены текущим заданием;
* merge PR, если задача заканчивается на Draft PR;
* действиями, которые невозможно безопасно отменить.

Если текущее задание прямо разрешает конкретное опасное действие и содержит условия его выполнения, дополнительное подтверждение не требуется. Сначала проверь все заданные preconditions и действуй строго по playbook.

### Не задавай лишних вопросов

Не спрашивай пользователя:

* какой файл открыть, если его можно найти поиском;
* какие тесты запустить, если это можно определить по diff;
* можно ли исправить очевидную ошибку в scope задачи;
* можно ли повторно запустить упавший тест;
* можно ли создать ветку или worktree;
* можно ли сделать commit или Draft PR, если это указано в задаче;
* продолжать ли после успешного промежуточного шага.

При небольшой неоднозначности выбери самый безопасный и минимальный вариант, явно укажи принятое допущение в итоговом отчёте и продолжай.

Задавай вопрос только когда без ответа высок риск:

* изменить не тот production-компонент;
* потерять данные;
* раскрыть секрет;
* выполнить необратимое действие;
* существенно выйти за scope;
* выбрать между несовместимыми продуктовыми требованиями.

### Правила работы с репозиторием

Не изменяй canonical checkout, если он dirty.

Используй отдельный clean worktree от указанного exact base SHA.

Перед изменениями проверяй:
...
Не рефактори соседние компоненты без необходимости.

### Работа с ошибками

Если команда завершилась ошибкой:

1. Самостоятельно прочитай ошибку.
2. Определи вероятную причину.
3. Выполни безопасную диагностику.
4. Исправь причину в пределах scope.
5. Повтори проверку.
6. Не проси пользователя копировать вывод, который уже доступен тебе.

Не сдавайся после первой ошибки.

При временной сетевой ошибке, timeout или недоступности remote выполни ограниченное число безопасных повторов. Затем остановись и сообщи точную блокирующую причину, не придумывая результат.

### Тестирование

Самостоятельно выбери:

* focused tests для изменённых компонентов;
* regressions для связанных функций;
* project-level check, если он предусмотрен repository.

Не запускай тяжёлые test suites параллельно на сервере с ограниченной памятью.
...
Не заявляй, что тест прошёл, если он не запускался или завершился ошибкой.

### Production safety

По умолчанию считай production неизменяемым.

Без прямого разрешения текущего задания:

* не build;
* не deploy;
* не restart;
* не recreate containers;
* не изменяй production DB;
* не меняй config;
* не очищай диск;
* не трогай Qdrant;
* не запускай ручные пользовательские действия от имени пользователя.

Read-only проверки production разрешены без подтверждения.
...
* «остановиться после Draft PR» — не merge;
* «остановиться после merge» — не build и не deploy;
* «read-only audit» — ничего не изменять;
...
Не присылай пошаговый поток мыслей.
...
Пиши `PASS`, `FAIL`, `NOT RUN`, `NOT PERFORMED` или `INCONCLUSIVE`.

Не подменяй неизвестное значение предположением.
