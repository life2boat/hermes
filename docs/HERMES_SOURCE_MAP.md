...
Verified against canonical main: `96611e1d7fa980e12f86f1ffcb1e56989c14aa18`
...
| Prompt engineering and quality | `docs/contracts/PROMPT_DESIGN_CONTRACT.md`, `docs/contracts/PROMPT_EVAL_CONTRACT.md`, `docs/contracts/PROMPT_FAILURE_TAXONOMY.md`, `docs/PROMPT_AUTHORING_GUIDE.md`, `docs/adr/ADR-0078-prompt-engineering-system.md`, `ai_engineering/prompt_contracts.py`, `ai_engineering/prompt_system.py`, `scripts/run_prompt_quality_evals.py`, `evals/prompt_quality/` | Structured PromptSpec, deterministic context selection/compilation/validation, untrusted-input isolation, versioned provenance, failure taxonomy, and provider-free regression evidence |
...
| Intent Control Plane | `docs/EFFECTIVE_POLICY_CONTRACT.md`, `docs/adr/ADR-0080-effective-policy-source-attribution.md`, `ai_engineering/task_intent.py`, `ai_engineering/requirements_gate.py`, `ai_engineering/task_analysis.py`, `ai_engineering/convergence.py`, `ai_engineering/effective_policy.py`, `scripts/prepare_task.py`, `scripts/clarify_task.py`, `scripts/check_requirements_gate.py`, `scripts/analyze_task.py`, `scripts/evaluate_convergence.py`, `scripts/explain_effective_policy.py` | Deterministic offline operator intent bounding (`TaskIntent`), lineage verification (`TaskLineage`), requirements quality gating, cross-artifact analysis, evidence-bound convergence, and exact Git-committed source attribution without authority expansion |
| Production Attestation | `docs/PRODUCTION_RUNTIME_ATTESTATION_V1.md`, `schemas/production-runtime-attestation-v1.schema.json`, `ai_engineering/production_runtime_attestation.py`, `ai_engineering/production_collectors/`, `scripts/production_runtime_attestation.py`, `scripts/collect_production_attestation.py` | Offline contracts, structural verification schemas, and exact-source read-only live collectors (Docker, SQLite, Qdrant, Secrets) for attestation and comparison |
| Production Readiness Evidence Bridge | `ai_engineering/production_readiness_evidence.py`, `schemas/production-readiness-evidence-v1.schema.json`, `scripts/check_production_readiness_evidence.py`, `tests/test_production_readiness_evidence.py` | Deterministic offline bridge from verified ProductionRuntimeAttestation/Comparison to PRODUCTION_READINESS_GATE. EVIDENCE_EXPANDS_AUTHORITY=false. |
...
| Memory Graph v3 | `ai_engineering/graph_contract.py`, `schemas/memory-graph-contract-v1.schema.json`, `tests/ai_engineering/test_graph_contract.py`, `docs/adr/ADR-0081-memory-graph-engineering-v3.md`, `gateway/memory/graph_projection.py`, `tests/gateway/memory/test_graph_projection.py`, `docs/adr/ADR-0082-deterministic-graph-projection-engine.md`, `gateway/memory/graph_query.py`, `tests/gateway/memory/test_graph_query.py`, `docs/adr/ADR-0084-deterministic-graph-read-path.md`, `ai_engineering/memory_graph_eval.py`, `scripts/run_memory_graph_evals.py`, `evals/memory_graph/`, `docs/adr/ADR-0086-memory-graph-evaluation-system.md`, `gateway/memory/graph_runtime.py`, `tests/gateway/memory/test_graph_runtime.py`, `docs/adr/ADR-0087-memory-graph-runtime-integration.md` | Deterministic graph contracts, strict authoritative (SQLite) to derived-graph boundary, canonical complete `O(n)` source-state freshness, pure structural projection queries, fail-closed graph validation, provenance-bound authoritative hydration, deterministic offline candidate evaluation, read-only adversarial evidence, and shadow runtime integration |
