...
def test_executor_success(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(process_identity, "read_poller_environ", lambda pid, identity, docker=None: b"TELEGRAM_BOT_TOKEN=synthetic_post_recreate_val\x00NORMAL=val\x00")
...
def test_executor_post_runtime_nameset_missing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("ops.secret_remediation_r1.preflight.run_compose_preflight", lambda: None)
    monkeypatch.setattr("ops.secret_remediation_r1.secret_transfer.resolve_poller_pid", lambda docker=None: (123, type("ID", (), {"container_id": "123"})))
    monkeypatch.setattr("ops.secret_remediation_r1.secret_transfer.read_poller_environ", lambda pid, identity, docker=None: b"TELEGRAM_BOT_TOKEN=secret\x00")
    monkeypatch.setattr("ops.secret_remediation_r1.compose_command.run_recreate", lambda *args, **kwargs: None)
    ...
    monkeypatch.setattr("ops.secret_remediation_r1.safe_fs.replace_existing_file", mock_publish)
    monkeypatch.setattr("ops.secret_remediation_r1.override_transform.publish_file", mock_publish)
    monkeypatch.setattr("ops.secret_remediation_r1.env_split.publish_file", mock_publish)
    monkeypatch.setattr("ops.secret_remediation_r1.secret_transfer.publish_file", mock_publish)
    monkeypatch.setattr("ops.secret_remediation_r1.process_identity.resolve_poller_pid", lambda docker=None: (123, type("ID", (), {"container_id": "123"})))
    
    # Missing TELEGRAM_BOT_TOKEN
    monkeypatch.setattr("ops.secret_remediation_r1.process_identity.read_poller_environ", lambda pid, identity, docker=None: b"NORMAL=val\x00")
    
    with pytest.raises(ExecutorError, match="Post-recreate protected NAME set mismatch"):
        run_remediation(str(base), str(override), docker=MockDockerBackend())
...
def test_executor_post_runtime_nameset_added(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("ops.secret_remediation_r1.process_identity.read_poller_environ", lambda pid, identity, docker=None: b"TELEGRAM_BOT_TOKEN=synthetic\x00DASHSCOPE_API_KEY=synthetic2\x00")
