...
import pytest
from ops.secret_remediation_r1.runtime_invariant import verify_runtime_invariants, capture_runtime_prestate, RuntimeInvariantError
...
def test_runtime_vector_changed_rejected():
    ...
    prestate = capture_runtime_prestate(backend_pre)
    data = get_base_data()
    data[0]["Config"]["Env"] = ["MEMORY_VECTOR_ENABLED=false", "QDRANT_ENDPOINT=http", "QDRANT_COLLECTION=healbite_memory_os"]
    backend_post = MockBackend(data)
    with pytest.raises(RuntimeInvariantError, match="MEMORY_VECTOR_ENABLED mismatch"):
        verify_runtime_invariants(expected=prestate, docker=backend_post)
...
def test_runtime_qdrant_changed_rejected():
    ...
    prestate = capture_runtime_prestate(backend_pre)
    data = get_base_data()
    data[0]["Config"]["Env"] = ["MEMORY_VECTOR_ENABLED=true", "QDRANT_ENDPOINT=https://new", "QDRANT_COLLECTION=healbite_memory_os"]
    backend_post = MockBackend(data)
    with pytest.raises(RuntimeInvariantError, match="QDRANT_ENDPOINT mismatch"):
        verify_runtime_invariants(expected=prestate, docker=backend_post)
