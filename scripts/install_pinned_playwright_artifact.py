...
def load_verified_manifest(path: Path, expected_sha256: str) -> dict[str, object]:
    ...
    metadata = _regular_file_metadata(path, "MANIFEST_FILE_INVALID")
...
def validate_manifest_contract(
    document: dict[str, object], contract: BrowserContract
) -> None:
    exact_matches = {
...
def _validate_entry_set(entries: list[ArchiveEntry]) -> None:
...
def _zip_entries(archive: zipfile.ZipFile) -> list[ArchiveEntry]:
...
def _tar_entries(archive: tarfile.TarFile) -> list[ArchiveEntry]:
    entries: list[ArchiveEntry] = []
    try:
...
def _extract_archive(archive_path: Path, archive_format: str, destination: Path) -> None:
...
def _prepare_cache_root(cache_root: Path) -> bool:
    ...
    try:
        ...
        try:
            parent = cache_root.parent.lstat()
...
def install_artifact(
    *,
    ...
    validate_manifest_contract(document, contract)
    ...
    try:
        ...
        try:
            marker.touch(mode=0o444, exist_ok=False)
            marker.chmod(0o444)
