        ...
        [
            {
                ...
                "Config": {
                    "Labels": {
                        ...
                        "com.docker.compose.service": "wrong",
                    }
                },
            }
        ],
...
def test_pid_zero_poller_reject(monkeypatch):
    ...
    monkeypatch.setattr(
        ops.secret_remediation_r1.process_identity, "_find_poller_processes", lambda: []
    )
...
def test_pid_multi_poller_reject(monkeypatch):
    ...
    monkeypatch.setattr(
        ops.secret_remediation_r1.process_identity,
        "_find_poller_processes",
        lambda: [100, 101],
    )
...
def test_revalidate_running_state_mutation(monkeypatch):
    ...
    identity = ContainerIdentity(
        container_id="cid", init_pid=1, image_id=LEGACY_IMAGE_ID, running=True
    )
    ...
    with pytest.raises(
        ProcessIdentityError, match="Container stopped during operation"
    ):
...
def test_revalidate_container_id_mutation(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda fd, n: (
            b""
            if hasattr(os, "_read_done")
            else (setattr(os, "_read_done", True) or b"data")
        ),
    )
    import ops.secret_remediation_r1.process_identity as pi
                ...
                return [
                    {
                        ...
                        "Config": {
                            "Labels": {
                                "com.docker.compose.project": "MUTATED",
                                "com.docker.compose.service": COMPOSE_SERVICE,
                            }
                        },
                    }
                ...
                return [
                    {
                        ...
                        "Config": {
                            "Labels": {
                                "com.docker.compose.project": COMPOSE_PROJECT,
                                "com.docker.compose.service": "MUTATED",
                            }
                        },
                    }
                ]
...
def test_revalidate_cgroup_mutation(monkeypatch):
    ...
    monkeypatch.setattr(pi, "_get_pid_namespace", lambda p: 100)
    monkeypatch.setattr(
        pi, "_read_pid_cgroup", lambda p: "cid" if docker.calls == 1 else "MUTATED"
    )
    monkeypatch.setattr(pi, "_find_poller_processes", lambda: [123])
...
def test_revalidate_second_poller_appeared(monkeypatch):
    ...
    with pytest.raises(
        ProcessIdentityError, match="Multiple pollers found during revalidation"
    ):
        read_poller_environ(pid=123, identity=identity, docker=docker)
