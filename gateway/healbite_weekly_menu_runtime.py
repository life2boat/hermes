...
WeeklyMenuStoreFactory = Callable[[], HealBiteWeeklyMenuStore]
...
class HealBiteWeeklyMenuRuntimeService:
    def __init__(
        ...
        *,
        ...
        weekly_menu_store_factory: WeeklyMenuStoreFactory | None = None,
    ) -> None:
    ...
    def _default_weekly_menu_store_factory(self) -> HealBiteWeeklyMenuStore:
        return HealBiteWeeklyMenuStore(db_path=self._db_path)
    ...
    def _resolve_store(self, actor_user_id: object) -> tuple[HouseholdAuthorizationContext, HealBiteWeeklyMenuStore]:
        context, availability = self._resolve_authorization_context(actor_user_id)
        try:
            store = self._weekly_menu_store_factory()
